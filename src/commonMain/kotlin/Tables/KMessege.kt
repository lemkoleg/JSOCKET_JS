...
@InternalAPI
private val KMessegeLock = Mutex()

@KorioExperimentalApi
@ExperimentalTime
@InternalAPI
val NEW_MESSEGES: ArrayDeque<ANSWER_TYPE> = ArrayDeque()
...
class KMessege() {
    ...
    fun setPROFILE_LINE(
        ...
        STRING_12 =
            l_messeges_type + l_messeges_access + l_messeges_status + l_have_full_text + l_have_answer + l_object_type
    }
    ...
    @JsName("setPROFILE_STRING")
    fun setPROFILE_STRING(v: String) {
        STRING_12 = v
    }
    ...
    companion object {
        ...
        @KorioExperimentalApi
        @JsName("ADD_NEW_MESSEGES")
        fun ADD_NEW_MESSEGES(): Promise<Boolean> =
            CoroutineScope(Dispatchers.Default).async {
            withTimeout(Constants.CLIENT_TIMEOUT) {
                try {
                    try {
                        KMessegeLock.lock()
                        while (NEW_MESSEGES.isNotEmpty()) {
                            TODO()
                        }
                        return@withTimeout true
                    } catch (ex: Exception) {
                        throw my_user_exceptions_class(
                            l_class_name = "KMessege",
                            l_function_name = "ADD_NEW_MESSEGES",
