    ...
    actual suspend fun SELECT_BIG_AVATAR(OBJECTS_ID: String): KBigAvatar? {
        ...
        if (res.next()) {
            kBigAvatar = KBigAvatar(
                L_AVATAR_ID = res.getString(1)!!,
                L_LAST_USE = res.getLong(2)!!,
                L_AVATAR = res.getBytes(3)!!
            )
        }
        return kBigAvatar
    }
    ...
    actual suspend fun SELECT_COMMANDS_ALL(): ArrayList<KCommands> {
        ...
        while (res.next()) {
            val kCommands = KCommands(
                L_COMMANDS_ID = res.getLong(1)!!.toInt(),
                L_COMMANDS_ACCESS = res.getString(2)!!,
                L_COMMANDS_PROFILE = res.getString(3)!!,
                L_OMMANDS_NECESSARILY_FIELDS = res.getString(4)!!,
                L_LAST_UPDATE = res.getLong(5)!!,
                L_COUNT_OF_EXECUTE = res.getLong(6)!!.toInt()
            )
            arr.add(kCommands)
        }
        return arr
    ...
    actual suspend fun SELECT_EXCEPTION_ALL(): ArrayList<KExceptions.KException> {
        ...
        while (res.next()) {
            val kException = KExceptions.KException(
                L_NAME_OF_ECXEPTION = res.getString(1)!!,
                L_LANG_OF_ECXEPTION = res.getString(2)!!,
                L_TEXT_OF_ECXEPTION = res.getString(3)!!,
                L_EXCEPTIONS_STATUS = res.getString(4)!!,
                L_LAST_UPDATE = res.getLong(5)!!
            )
            arr.add(kException)
        }
    ...
    actual suspend fun SELECT_ALL_METADATA(): ArrayList<KMetaData> {
        ...
        while (res.next()) {
            val kMetaData = KMetaData(
                L_VALUE_NAME = res.getString(1)!!,
                L_VALUE_VALUE = res.getLong(2)!!,
                L_LAST_UPDATE = res.getLong(3)!!
            )
            arr.add(kMetaData)
        }
        return arr
    ...
    actual suspend fun SELECT_SAVEMEDIA_ALL(conn_id: Long): ArrayList<KSaveMedia> {
        ...
        while (res.next()) {
            val kSaveMedia = KSaveMedia(
                L_OBJECT_LINK = res.getString(1)!!,
                L_OBJECT_SIZE = res.getLong(2)!!,
                L_OBJECT_LENGTH_SECONDS = res.getLong(3)?.toInt() ?: 0,
                L_OBJECT_EXTENSION = res.getString(4)!!,
                L_AVATAR_ID = res.getString(5),
                L_IS_TEMP = res.getLong(6)?.toInt() ?: 1,
                L_LAST_USED = res.getLong(7)!!
            )
            arr.add(kSaveMedia)
        }
        return arr
    ...
    actual suspend fun SELECT_CASHLASTUPDATE_ALL(): ArrayList<KCashLastUpdate> {
        ...
        while (res.next()) {
            val kCashLastUpdate = KCashLastUpdate(
                CASH_SUM = res.getString(2)!!,
                OBJECT_ID = res.getString(3)!!,
                RECORD_TYPE = res.getString(4)!!,
                COURSE = res.getString(5)!!,
                SORT = res.getString(6)!!,
                LINK_OWNER = res.getString(7)!!,
                MESS_COUNT_FROM = res.getString(8)!!,
                OTHER_CONDITIONS_1 = res.getString(9)!!,
                OTHER_CONDITIONS_2 = res.getString(10)!!,
                OTHER_CONDITIONS_3 = res.getString(11)!!
            )
